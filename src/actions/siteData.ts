...
import { HEADER_QUERY, FOOTER_QUERY, SITE_SETTINGS_QUERY, COMPANY_LINKS_QUERY, CONTACT_FORM_SETTINGS_QUERY, LEGAL_PAGES_VISIBILITY_QUERY, CONTACT_GENERAL_CONTENT_QUERY, CONTACT_CONFIRMATION_EMAIL_QUERY, APPLY_PAGE_QUERY, APPLY_PRIVACY_STATEMENT_QUERY, APPLY_QUESTIONNAIRE_QUERY } from '@/sanity/lib/queries';
import type { FOOTER_QUERYResult, HEADER_QUERYResult, SITE_SETTINGS_QUERYResult, COMPANY_LINKS_QUERYResult, CONTACT_FORM_SETTINGS_QUERYResult, LEGAL_PAGES_VISIBILITY_QUERYResult, CONTACT_GENERAL_CONTENT_QUERYResult, CONTACT_CONFIRMATION_EMAIL_QUERYResult, APPLY_PAGE_QUERYResult, APPLY_PRIVACY_STATEMENT_QUERYResult, APPLY_QUESTIONNAIRE_QUERYResult } from '@/sanity/types';
...
export async function getApplyPrivacyStatement(): Promise<APPLY_PRIVACY_STATEMENT_QUERYResult | null> {
  const { data } = await sanityFetch({
...
export async function getApplyQuestionnaire(): Promise<APPLY_QUESTIONNAIRE_QUERYResult | null> {
  const { data } = await sanityFetch({
