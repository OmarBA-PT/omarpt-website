        ...
        {items.map((item, index) => {
          ...
          return (
            <div
              ...
              {/* Icon */}
              <div
                className={`flex-shrink-0 ${isHorizontal ? '' : 'text-center'}`}
                {...(documentId && documentType
                  ? createSanityDataAttribute(documentId, documentType, `${itemPath}.icon`)
                  : {})}>
                <Icon iconKey={item.icon as IconKey} className='text-black' size={48} />
              </div>

              {/* Description */}
              <div className={`flex-1 ${isHorizontal ? '' : 'text-center'}`}>
                <p
                  {...(documentId && documentType
                    ? createSanityDataAttribute(documentId, documentType, `${itemPath}.description`)
                    : {})}
                  className='text-body-3xl'>
                  {item.description}
                </p>
              </div>
            </div>
